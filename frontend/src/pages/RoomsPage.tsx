...
import { Plus, X, Edit2, Trash2, Users, Home, BedDouble, Crown, Baby } from 'lucide-react';
...
const ROOM_STATUSES: RoomStatus[] = ['AVAILABLE', 'OCCUPIED', 'RESERVED', 'CLEANING', 'MAINTENANCE', 'DIRTY'];

/* ─── Room type icon + color mapping ───────────────────── */
...
export default function RoomsPage() {
  ...
  const [typeFormData, setTypeFormData] = useState({ name: '', description: '', basePrice: 0, maxOccupancy: 2, amenities: '' });
  ...
  const handleSubmitRoom = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (editingRoom) { await updateRoom.mutateAsync({ id: editingRoom.id, ...formData }); }
      else { await createRoom.mutateAsync(formData); }
      setShowRoomForm(false);
    } catch (err: any) { alert(err.response?.data?.error || 'Failed to save room'); }
  };
  ...
  const handleSubmitType = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTypeFormData({ name: '', description: '', basePrice: 0, maxOccupancy: 2, amenities: '' });
    } catch (err: any) { alert(err.response?.data?.error || 'Failed to create room type'); }
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this room?')) return;
    try { await deleteRoom.mutateAsync(id); } catch (err: any) { alert(err.response?.data?.error || 'Failed to delete room'); }
  };
                ...
                {roomsByFloor[floor]?.map((room, idx) => {
                  ...
                  return (
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      onKeyDown={(e) => e.key === 'Enter' && openEdit(room)}
                    ...
                    >
                      ...
                      {/* Type icon */}
                      <div className="room-card-icon" style={{ background: meta.bg, color: meta.color }}>
                        {meta.icon}
                      </div>
                      ...
                      {/* Occupancy */}
                      <div className="room-card-occ">
                        <Users className="w-3 h-3" style={{ opacity: 0.5 }} />
                        {occ}
                      </div>

                      {/* Price */}
                      <div className="room-card-price">{formatCurrency(Number(room.roomType?.basePrice))}/night</div>
              ...
              return (
                <div key={type.id} style={{ padding: '16px', borderRadius: '12px', background: '#f9f7f4', border: '1px solid #ece9e1', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-hotel-gold)' }}>{formatCurrency(Number(type.basePrice))}<span style={{ fontSize: '11px', fontWeight: 400 }}>/night</span></span>
        ...
        <div className="modal-overlay" onClick={() => setShowRoomForm(false)}>
          <div className="modal-card" style={{ maxWidth: '480px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmitRoom} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div><label className="form-label">Room Type</label><select value={formData.roomTypeId} onChange={(e) => setFormData(p => ({ ...p, roomTypeId: e.target.value }))} className="form-select" required><option value="">Select room type</option>{roomTypes?.map(t => <option key={t.id} value={t.id}>{t.name} ({formatCurrency(Number(t.basePrice))})</option>)}</select></div>
        ...
        <div className="modal-overlay" onClick={() => setShowTypeForm(false)}>
          <div className="modal-card" style={{ maxWidth: '480px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmitType} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div><label className="form-label">Base Price</label><input type="number" value={typeFormData.basePrice} onChange={(e) => setTypeFormData(p => ({ ...p, basePrice: parseFloat(e.target.value) }))} className="form-input" required /></div>
